...
class ProjectStore:
    ...
    def ensure_contract_nars(
        self, project_id: str, mission_id: str, assertion_id: str
    ) -> str:
        """Load contract with NARS conversion.

        If the contract .md file exists but lacks NARS header, convert the
        JSON assertion (if exists) to NARS header format and update the .md file.

        Returns the contract content with NARS header.
        """
        import json
        from .contractreifier import render_contract_head, render_full_with_contract

        contract_md = self.contract_assertion_path(project_id, mission_id, assertion_id)
        contract_json = self.mission_dir(project_id, mission_id) / "assertions" / f"{assertion_id}.json"

        # If .md doesn't exist but .json does, create from scratch
        if not contract_md.exists() and contract_json.exists():
            json_bytes = contract_json.read_bytes()
            data = json.loads(json_bytes)

            # Extract NARS from JSON if present
            nars = data.get("nars", [])

            # Build a minimal contract from the JSON
            title = data.get("title", data.get("id", assertion_id))
            body = data.get("description", data.get("body", ""))

            contract_text = f"""# Contract: {title}
...
"""
            ...
            contract_md.write_text(contract_text, encoding="utf-8")
            return contract_text

        # If .md exists, check if it has NARS header
        if contract_md.exists():
            content = contract_md.read_text(encoding="utf-8")

            # Check if already has NARS section
            if "## NARS" in content or "\n- " in content:
                return content

            # Need to add NARS - if .json exists, convert it
            if contract_json.exists():
                ...
                if nars:
                    # Convert JSON to NARS header format
                    nars_header = "## NARS\n" + "\n".join(f"- {n}" for n in nars)

                    # Append NARS to existing content
