...
@click.argument("project_id")
@click.option("--mission", "mission_id", default=None)
def metrics_cmd(project_id: str, mission_id: str | None) -> None:
    ...
    """
    from .missiondata import collect_mission_metrics
    from .storage import ProjectStore
    import json
    ...
    if mission_id is not None:
        ...
        try:
            missions = store.list_missions(project_id)
        except Exception:
            raise click.ClickException(f"Project not found or error: {project_id}")
    ...
    for mid in missions:
        try:
            metrics = collect_mission_metrics(store, project_id, mid)
        except Exception as exc:
            continue

        # 1. MISSION block
        p_count = metrics.task_counts.get("pending", 0)
        a_count = metrics.task_counts.get("active", 0)
        c_count = metrics.task_counts.get("completed", 0)
        f_count = metrics.task_counts.get("failed", 0)
        s_count = metrics.task_counts.get("superseded", 0)
        
        c_passed = sum(1 for c in metrics.contracts if c.verdict == "passed")
        c_failed = sum(1 for c in metrics.contracts if c.verdict == "failed")
        c_pending = sum(1 for c in metrics.contracts if c.verdict == "pending")

        mission_block = [
            f'{{"id":{json.dumps(mid)},"type":"mission","total_tasks":{len(metrics.tasks)}}}',
            f'task_counts: pending={p_count}, active={a_count}, completed={c_count}, failed={f_count}, superseded={s_count}',
            f'attempts: total={metrics.total_attempts}, successes={metrics.total_successes}',
            f'verdicts: passed={c_passed}, failed={c_failed}, pending={c_pending}',
            "", "", "", "", "", ""
        ]
        click.echo("\n".join(mission_block[:10]))
        click.echo("$ ---")

        # 2. CONTRACT blocks
        for c in metrics.contracts:
            nars_lines = []
            contract_file = store.zenith_dir(project_id) / mid / "contract" / f"{c.assertion_id}.md"
            if contract_file.exists():
                try:
                    for line in contract_file.read_text(encoding="utf-8").splitlines():
                        line_stripped = line.strip()
                        if line_stripped.startswith("- ") or line_stripped.startswith("* "):
                            nars_lines.append(line_stripped[2:])
                except Exception:
                    pass

            c_block = [
                f'{{"id":{json.dumps(c.assertion_id)},"type":"contract","verdict":{json.dumps(c.verdict)}}}',
                f'owner_task: {c.owner_task_id or "none"} ({c.owner_status or "none"})',
            ]
            for term in nars_lines[:7]:
                c_block.append(f'  {term}')
            while len(c_block) < 10:
                c_block.append("")
            click.echo("\n".join(c_block[:10]))
            click.echo("$ ---")
        ...
        for t in metrics.tasks:
            t_block = [
                f'{{"id":{json.dumps(t.task_id)},"type":"task","status":{json.dumps(t.status)},"task_type":{json.dumps(t.task_type)}}}',
                f'depends_on: {json.dumps(t.depends_on)}',
                f'targets: {json.dumps(t.targets)}',
                f'skill: {t.skill or "none"}',
                f'attempts: count={t.attempt_count}, successes={t.success_count}',
                "", "", "", "", ""
            ]
            click.echo("\n".join(t_block[:10]))
            click.echo("$ ---")
