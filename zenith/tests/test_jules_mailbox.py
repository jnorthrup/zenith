...
def test_mailbox_uses_canonical_repo_root_through_symlink(tmp_path: Path) -> None:
    ...
    save_jules_session(str(link), "123", "task-1", "project-1", "mission-1")

    assert find_git_root(str(link)) == repo.resolve()
    assert session_store_path(str(link)) == repo.resolve() / ".zenith" / "mailbox" / "jules_sessions.json"
    assert jules_mailbox_path(str(link), "123") == repo.resolve() / ".zenith" / "mailbox" / "jules" / "123.jsonl"

    store = load_session_store(str(link))
...
def test_append_mailbox_records_events_and_updates_session_index(tmp_path: Path) -> None:
    ...
    save_jules_session(str(repo), "abc/123", "task-1", "project-1", "mission-1")
    ...
    store = load_session_store(str(repo))
