...
% IP notice block reused on the cover, as a Section 2 footnote, and in the appendix.
\newcommand{\ipnoticebody}{\textit{\textbf{Intellectual Property Notice}: The browser-based physics game task referred to in this Technical Report as ``ii-sling-physics-game'' was developed internally by Intelligent Internet (L42 Ltd.) as a software engineering benchmark for evaluating long-horizon agent harness performance. It references third-party intellectual property solely for the purpose of describing benchmark tasks. The game concept used as the basis for this benchmark is inspired by a third-party commercial game. No third-party assets, code, or proprietary materials were used in the development of the benchmark implementation. Any resemblance to third-party intellectual property is incidental to the benchmarking purpose. This Technical Report is not affiliated with or endorsed by any third-party game developer or publisher. All third-party trade marks and copyrights referenced herein remain the property of their respective owners. Intelligent Internet (L42 Ltd.) makes no claim of ownership over any third-party intellectual property referenced in this report.}}
...
% IP Notice on the cover page (per Legal's recommendation).
{\footnotesize\color{muted}\ipnoticebody\par}
...
\midrule
\endhead
Product builds & \code{ii-sling-physics-game}\footnotemark[1] & Internal & Build a browser game from a multi-file specification with level references and fidelity constraints. & Carry many linked gameplay, visual, physics, and content requirements across a long implementation path. \\
\rowcolor{tableband}
...
% IP notice footnote anchored to the first \code{ii-sling-physics-game} mention above.
\footnotetext[1]{\ipnoticebody}
...
\rowcolor{tableband}
\code{ii-sling-physics-game} & One RALPH session called the game feature-complete, but a later session still found concrete gameplay and level-fidelity gaps. & Reopening the task can surface missing requirements after a self-reported done state. \\
...
Detailed clips are in Appendix~A. The scores move in the same direction on these examples: Claude \code{Git-convert} improved from \code{0.2067} to \code{0.287}, \code{ii-sling-physics-game} from \code{0.30} to \code{0.55}, and \code{ydata-profiling} from \code{0.009166} to \code{0.06141}.
...
\rowcolor{tableband}
\code{ii-sling-physics-game} milestone 3 & Focused gameplay checks passed, but the independent verifier still caught a build regression. & Independent testing can catch regressions outside the worker's local checks. \\
...
\rowcolor{tableband}
\code{ii-sling-physics-game} & The run separates game-engine work from content work and synthesizes reusable guidance for later sessions. & Skills and worker roles can preserve reusable knowledge. \\
...
\subsubsection{ii-sling-physics-game}
...
Compared all 30 levels against their internal reference images using parallel agents.
...
Critical bug: when all target characters were defeated exactly at the 900ms `loseTransition`
...
\subsubsection{ii-sling-physics-game: build regression caught after gameplay checks passed}
...
\textbf{ii-sling-physics-game: planning from an empty spec repo before workers start.}
...
validated before later chapters build on them.

Milestone: core-playable-slice
- Vite + Phaser 3 + Matter.js game shell, scene/state structure, slingshot
  aiming and launch, camera behavior, target/block/hazard resolution, score/win/lose
  loop, and a stable window.gameTestAPI.

Milestone: chapter-1-campaign
- Menu and level-select shell, sequential unlock, in-session best score/star
  retention, all chapter-1 levels, and the Type-A/Type-B character gameplay needed.
...
\subsubsection{ii-sling-physics-game: adaptive worker skills and reusable guidance}
...
description: Implements core game engine features - physics, mechanics, systems,
  UI, audio, and game logic for the ii-sling-physics-game browser game.
\end{cbox}

\begin{cbox}
name: game-content-worker
description: Creates game content - level data, sprites, and audio assets for the
  ii-sling-physics-game browser game using internal reference images and specification sources.
\end{cbox}
...
\begin{cbox}
{
  ...
  "appliedUpdates": [
    { "target": "library",
      ...
      "sourceFeature": "blocks-and-targets, slingshot-mechanics" }
  ],
...
\subsection*{ii-sling-physics-game}
